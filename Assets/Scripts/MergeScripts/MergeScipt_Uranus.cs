...
public class MergeScipt_Uranus : MonoBehaviour
{
    int id;
    ...
    {
        if (col.gameObject.CompareTag("Uranus"))
        {
            if (col.gameObject.GetComponent<MeshRenderer>().material.color == GetComponent<MeshRenderer>().material.color)
            {
                if (id < col.gameObject.GetComponent<MergeScipt_Uranus>().id)
                {
                    return;
                }
                Destroy(col.gameObject);
                Destroy(gameObject);
                ScoreScript.instance.AddPointUranus();
